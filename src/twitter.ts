...
export const syncTwitter = async (client: Client) => {
  ...
  const {
    data: { id },
  } = await twitterApp.v2.userByUsername(process.env.TWITTER_USERNAME);
  ...
  await twitterApp.v2.updateStreamRules({
    add: [
      ...
      {
        value: `@${process.env.TWITTER_USERNAME} has:mentions`,
        tag: 'mention',
      },
      ...
      if (discordId) {
        if (eventData.matching_rules.some((rule) => rule.tag === 'response')) {
          await grantPoints(
            ...
            50,
            (client as Accountant).pool
          );

          console.log(
            `${discordId} added a response and was granted 10 points!`
          );
          const user =
            client.users.cache.get(discordId) ??
          ...
          (client.channels.cache.get('997899324052750478') as TextChannel).send(
            {
              embed: new MessageEmbed()
                ...
                .setDescription(
                  `${user.tag} added a Twitter response and was granted 10 points!`
                )
                .setColor('#00ff00')
                .setTimestamp(),
            }
        ...
        if (eventData.matching_rules[0]?.tag === 'mention') {
          ...
          console.log(
            `${discordId} added a mention and was granted 10 points!`
          );
          ...
          (client.channels.cache.get('997899324052750478') as TextChannel).send(
            {
              embed: new MessageEmbed()
                .setAuthor(user.tag, user.displayAvatarURL())
                .setDescription(
                  `${user.tag} added a Twitter mention and was granted 10 points!`
                )
                .setColor('#00ff00')
                .setTimestamp(),
            }
        ...
        if (eventData.matching_rules[0]?.tag === 'retweet') {
          ...
          console.log(
            `${discordId} added a retweet and was granted 10 points!`
          );
          ...
          (client.channels.cache.get('997899324052750478') as TextChannel).send(
            {
              embed: new MessageEmbed()
                .setAuthor(user.tag, user.displayAvatarURL())
                .setDescription(
                  `${user.tag} added a Twitter retweet and was granted 10 points!`
                )
                .setColor('#00ff00')
                .setTimestamp(),
            }
